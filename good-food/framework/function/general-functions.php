...
function good_food_register_css_styles() {
	if (!is_admin()) {
		...
		if (is_rtl()) {
			wp_enqueue_style('good-food-rtl', get_template_directory_uri() . '/theme-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/css/rtl.css', array(), '1.0.0', 'screen, print');
		}
...
function good_food_register_js_scripts() {
	...
	if ( 
		is_a($post, 'WP_Post') && 
		(strpos($post->post_content, '[cmsmasters_portfolio ') || strpos($post->post_content, '[cmsmasters_gallery '))
	) {
